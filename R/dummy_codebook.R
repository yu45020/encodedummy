create_code_book = function(dat){
  # create a code book that has one-on-one match for non-numeric values for each col
...
update_code_book = function(code_book, dat){
  ...
  for(col in cols){
    ...
    if(identical(new_unique_f, origin_book_f)){
      ...
      # update code book
      new_length = length(origin_book_f)

      code_book[[col]] = data.table(digits = 1:new_length)
      code_book[[col]][,(col):=origin_book_f]

      setcolorder(code_book[[col]] , c(col, 'digits'))
      if((new_length-old_length)>0){
        print(sprintf("Column %s has %i new unique values.", col, new_length-old_length))
      }else{
        print(sprintf("Column %s has no new unique values", col))
      }
...
onehot_encoder = function(dat,code_book=NULL,
                          convert_to='numeric',
                          update_book = FALSE){
